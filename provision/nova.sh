...
then
  ...
  crudini --set "${NOVA_CFG}" DEFAULT sql_connection "mysql://${DB_USER}:${DB_PASSWORD}@${MGMT_IP}/nova"
  # quantum
...
crudini --set "${COMPUTE_CFG}" libvirt virt_type qemu
...
then
  ...
  do
    ...
    mysql -u "${DB_USER}" --password="${DB_PASSWORD}" -e "delete from ${t}" nova
    if [[ $? == 0 ]]
    then
